...
export default function ConversationComponent({
  ...
  const [candidateState, setCandidateState] = useState<CandidateState | null>(
    null,
  );
  const assessedTurnsRef = useRef(new Set<string>());
  ...
  useEffect(() => {
    ...
    const recentContext = messageList
      .slice(-6)
      .map((message) => {
        const speaker = String(message.uid) === agentUID ? 'Interviewer' : 'Candidate';
    ...
    for (const message of messageList) {
      ...
      const turnId = String(message.turn_id ?? `${message.uid}-${message.createdAt}-${text.slice(0, 24)}`);
      if (assessedTurnsRef.current.has(turnId)) continue;
      assessedTurnsRef.current.add(turnId);
      ...
      void fetch('/api/assess', {
        ...
        .then(async (response) => {
          ...
          const body = (await response.json()) as { state?: CandidateState };
          if (body.state) setCandidateState(body.state);
        })
        .catch((error) => {
          console.error('Failed to assess turn:', error);
        });
    }
  }, [messageList, agoraData.agentId, agentUID]);
    ...
    <QuickstartConversationLayout
      ...
      transcriptPanel={
        <QuickstartTranscriptPanel
          ...
          agentUID={agentUID}
          scoreStrip={<LiveScoreStrip state={candidateState} />}
        />
      }
      visualizer={
        <div
