...
- `prompts.system`: edit `INTERVIEW_PANEL_PROMPT` and `GREETING` in `app/api/invite-agent/route.ts`.
...
- Required env vars are `NEXT_PUBLIC_AGORA_APP_ID`, `NEXT_AGORA_APP_CERTIFICATE`, and `NEXT_GEMINI_API_KEY`.
