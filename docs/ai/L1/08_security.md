...
- Prompt includes mandatory AI disclosure and an honesty rule against inventing candidate background.
...
- Set `NEXT_PUBLIC_AGORA_APP_ID` and `NEXT_AGORA_APP_CERTIFICATE` as before.
- Also set `NEXT_GEMINI_API_KEY` as a server-side secret when deploying this fork.
