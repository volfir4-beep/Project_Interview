...
- `NEXT_GEMINI_API_KEY`: Google AI Studio key for Gemini BYOK (server only).

Agent behavior (interview-panel prompt, native Gemini vendor, VAD) lives in `app/api/invite-agent/route.ts`.
