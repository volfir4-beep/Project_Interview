...
- Default agent config: Agora-managed Deepgram STT and MiniMax TTS; Gemini BYOK via Agora's native `Gemini` vendor (`style: "gemini"`). Scoring JSON still uses Google's OpenAI-compatible endpoint. `.env.local` needs Agora credentials plus `NEXT_GEMINI_API_KEY`.
...
- Set `NEXT_PUBLIC_AGORA_APP_ID`, `NEXT_AGORA_APP_CERTIFICATE`, and `NEXT_GEMINI_API_KEY` in the deployment target.
- Keep `NEXT_AGORA_APP_CERTIFICATE` and `NEXT_GEMINI_API_KEY` server-side only.
