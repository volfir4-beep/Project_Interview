...
agora project use <your-project>
agora project env write .env.local
rg "^(NEXT_PUBLIC_AGORA_APP_ID|NEXT_AGORA_APP_CERTIFICATE|NEXT_GEMINI_API_KEY)=" .env.local
```

Copy those values into Vercel Project Settings -> Environment Variables. Keep `NEXT_AGORA_APP_CERTIFICATE` and `NEXT_GEMINI_API_KEY` server-only.
...
| `NEXT_GEMINI_API_KEY`        |    ✅    | Google AI Studio API key. **Server-side only.** Used by `/api/invite-agent` for Gemini BYOK. |

This EchoSphere fork uses Agora-managed Deepgram STT and MiniMax TTS, plus Gemini BYOK for the LLM. Keep real keys in `.env.local` only.
